...
def send_discord_notification(course_name, details, is_open=True):
    """Sends an embedded alert to your Discord channel."""
    if not DISCORD_WEBHOOK_URL:
        print("❌ Error: DISCORD_WEBHOOK secret is not set.")
        return

    # If open, ping @everyone with a loud color. If full, send quietly.
    content_tag = "@everyone 🚨 **Class Spot Alert!**" if is_open else "ℹ️ **Class Status Update**"
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not found_open_section:
                ...
                # Send quiet notification when all sections remain full
                send_discord_notification(
                    COURSE_CODE,
            ...
            send_discord_notification(
                COURSE_CODE,
                f"⚠️ Script ran into an error loading Self-Service: `{e}`",
                is_open=False
            )
        finally:
            browser.close()
